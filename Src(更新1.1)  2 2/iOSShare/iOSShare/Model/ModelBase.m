...
//-(void)dealloc
//{
//	DDLogInfo(@"Model:%@ dealloc susess",self);
//    [super dealloc];
//}
...
-(void)setValueForProperty:(objc_property_t)property Value:(id)value
{
    ...
    if (splitarray.count>0) {
        ...
        if ([first hasPrefix:@"@"]) {//是一个其他类型
			...
			}else if([first isEqualToString:@"B"]){//BOOL 类型,在64bit的时候会被解析为B 接受NSNumber类型 nsstring
				if ([value isKindOfClass:[NSNumber class]]||[value isKindOfClass:[NSString class]]) {
					BOOL boolValue=[value boolValue];
					
					BOOL *_set=ivar_pointer;
					*_set=boolValue;
				}
            ...
            }else if ([first isEqualToString:@"l"] || [first isEqualToString:@"q"]){//long类型  接受NSNumber类型 nsstring (64位时为q)
                if ([value isKindOfClass:[NSNumber class]]||[value isKindOfClass:[NSString class]]) {
                    long longValue;
...
-(NSDictionary*)dictionary
{
	...
	while (c!=[ModelBase class]) {
		...
		for (int i=0; i<procount; i++) {
			...
			//        }
			id value=[self serializableValueForProperty:pro];
			if (value!=nil) {
				[dic setObject:value forKey:name];
			}
		}
...
//                return _S(@"%hd",*((short*)ivar_pointer));
			...
			}else if ([first isEqualToString:@"B"]){//BOOL类型 BOOL在64bit时会被解析为B 接受NSNumber类型 nsstring
				//                return _S(@"%hd",*((short*)ivar_pointer));
				return @(*((BOOL*)ivar_pointer));
