...
/**
 标记PRTableView即将开始加载更多
 */
UIKIT_EXTERN NSString * const kPRTableViewWillBeginLoadingMoreNotification;
/**
 标记PRTableView完成加载更多
 */
UIKIT_EXTERN NSString * const kPRTableViewDidEndLoadingMoreNotification;
...
/**
 ...
 同时会向通知中心发送一个通知
 调用此函数（如果使用了自动调整更多视图位置，会导致表格调用delegate自动重新计算moreFooterView的高度）
 @param userInfo : 发送通知的时候使用的userInfo
 */
