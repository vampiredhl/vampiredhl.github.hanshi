...
@property (weak, nonatomic) HSImageView *imageView;

@end

@interface UIImageViewD : UIImageView

@end

@implementation UIImageViewD
...
- (void)displayImage:(NSString*)image {
    ...
    HSImageView *imageView = [[HSImageView alloc] initWithFrame:self.bounds];
//	if ([image hasPrefix:@"http://"]) {
//		[imageView sd_setImageWithURL:URL(image)];
//	}else{
//		[imageView setImage:[UIImage imageWithContentsOfFile:image]];
//	}
	[imageView setImageWithString:image placeholderImage:nil];
...
//        yScale = MIN(1, yScale);
		...
		minScale = xScale;//MIN(xScale, yScale);
    }
